...
export class IncidenciaService implements servicioIncidenciaI {
    ...
    async modificarEstadoIncidencia(id: number, state: number): Promise<number> {
        const IncidenciaModificada: number = await this.incidenciarepository.actualizarEstado(id, state);

        return IncidenciaModificada;
    }

    async eliminarIncidencia(id: number): Promise<number> {
        const IncidenciaEliminada: number = await this.incidenciarepository.eliminar(id);

        return IncidenciaEliminada;
    }

    async obtenerTodasIncidencias(): Promise<Incidencia[]> {
        const IssuesObtenidas: Issue[] = await this.incidenciarepository.obtenerTodas();
        const IncidenciasObtenidas: Incidencia[] = IssuesObtenidas.map(function(IssueObtenida) {
            var incidenciaprops: IncidenciaProps = {
