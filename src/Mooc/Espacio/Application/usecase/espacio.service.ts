...
export class EspacioService implements servicioEspacioI {
  ...
  async importarEspacios(): Promise<boolean> {
    ...
    const InsertarEspaciosPromise = new Promise<InsertResult>(
      (resolve, reject) => {
        fs.createReadStream(
          './src/Mooc/Espacio/Application/usecase/TB_ESPACIOS.csv',
        )
          .on('error', (err) => {
            reject(err);
          })
          ...
          .on('end', async () => {
            ...
            try {
              const resultadoOperacion =
                await this.espaciorepository.importarEspacios(espacios);
    ...
    try {
      ...
      switch (error.code) {
        ...
        default:
          console.error(
            'Error al insertar espacios en la Base de datos, mensaje de error: ',
            error.message,
          );
          break;
      }
