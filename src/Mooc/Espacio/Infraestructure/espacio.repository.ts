...
export class EspacioRepoPGImpl implements EspacioRepository {
  public repositorioEspacios: Repository<Space>;

  constructor(@Inject('DataSrc') private datasrcI: DataSource) {
    returnRepositoryTest(Space, this.datasrcI).then((repo) => {
      this.repositorioEspacios = repo;
    });
  }

  private mapaTraduccionQueries: {
    [key: number]: { query: string; parameters: string[] };
  } = {
    0: {
      query: EspacioQueries.QUERY_FILTRAR_ESPACIOS,
      parameters: ['capacity', 'floor', 'building'],
    },
    1: {
      query: EspacioQueries.QUERY_FILTRAR_ESPACIOS_FECHA,
      parameters: ['capacity', 'floor', 'building', 'fecha'],
    },
    2: {
      query: EspacioQueries.QUERY_FILTRAR_ESPACIOS_HORA,
      parameters: ['capacity', 'floor', 'building', 'hora'],
    },
    3: {
      query: EspacioQueries.QUERY_FILTRAR_ESPACIOS_FECHA_HORA,
      parameters: ['capacity', 'floor', 'building', 'fecha', 'hora'],
    },
  };
  ...
  async buscarEspacioPorId(id: string): Promise<Space> {
    const EspacioObtenido: Space = await this.repositorioEspacios.findOne({
      where: {
        id: id,
      },
    });
  ...
  async filtrarEspaciosReservables(
    ...
    const EspaciosObtenidos: Space[] = await this.repositorioEspacios
  ...
  async importarEspacios(espacios: Espacio[]): Promise<InsertResult> {
    ...
    const espaciosImportados: InsertResult =
      await this.repositorioEspacios.insert(spacesDTO);
    return espaciosImportados;
  }
}

function sustituirParametros(
  parameters: any[],
  espacio: Espacio,
  fecha: string,
  hora: number,
