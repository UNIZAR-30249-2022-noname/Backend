...
export class ReservaRepoPGImpl implements ReservaRepository {
    ...
    async guardar(reserva: Reserva): Promise<boolean> {
        ...
        var resultadoQuery = await client.query(ReservaQueries.QUERY_INTRODUCIR_RESERVA, [idEspacioReserva,datosReserva.HoraInicio,datosReserva.HoraFin,datosReserva.Fecha,datosReserva.Persona])
        //Analizar resultados devolver una cosa u otra
        client.release()
        return resultadoQuery.rowCount > 0;
    }
    async actualizar(id: string, hourstart: string, hourend: string, date: string): Promise<boolean> {
        var client = await poolConn.connect();
        var resultadoQuery = await client.query(ReservaQueries.QUERY_ACTUALIZAR_RESERVA, [hourstart,hourend,date,id])
        //Analizar resultados devolver una cosa u otra
        client.release()
        return true;
    }
    async eliminar(id: string): Promise<boolean> {
        var client = await poolConn.connect();
        var resultadoQuery = await client.query(ReservaQueries.QUERY_ELIMINAR_RESERVA, [id])
        //Analizar resultados devolver una cosa u otra
        client.release()
        return true;
    }
    ...
    async buscarReservaPorId(id: string): Promise<Boolean> {
        var client = await poolConn.connect();
        var resultadoQuery = await client.query(ReservaQueries.QUERY_BUSCAR_RESERVA_POR_ID, [id])
        //Analizar resultados devolver una cosa u otra
        client.release()
