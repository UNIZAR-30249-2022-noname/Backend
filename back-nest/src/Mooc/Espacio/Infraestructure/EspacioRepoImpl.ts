...
export class EspacioRepoPGImpl implements EspacioRepository { 
    async guardar(espacio: Espacio): Promise<boolean> {
        ...
        var resultadoQuery = await client.query(EspacioQueries.QUERY_INTRODUCIR_ESPACIO, [datosEspacio,datosEspacio.Name,datosEspacio.Capacity,datosEspacio.Building,datosEspacio.Kind])
    ...
    async buscarEspacioPorId(id: String): Promise<Espacio> {
        var client = await poolConn.connect();
        var resultadoQuery = await client.query(EspacioQueries.QUERY_BUSCAR_ESPACIO_POR_ID, [id])
        client.release();
